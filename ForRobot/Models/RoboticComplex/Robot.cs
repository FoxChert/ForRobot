...
namespace ForRobot.Models.RoboticComplex
{
    public class Robot : INotifyPropertyChanged, IDisposable
    {
        ...
        private List<ForRobot.Models.Controls.IFile> FilesCollection = new List<Controls.IFile>();
        ...
        /// Файлы роботов
        /// </summary>
        public ForRobot.Models.Controls.File Files { get; }
        ...
        private void LoadFiles(List<ForRobot.Models.Controls.IFile> data = null, ForRobot.Models.Controls.IFile node = null, int index = 1)
        {
            if (!this.IsConnection)
                return;
            ...
            if (data == null)
            {
                this.FilesCollection = new List<ForRobot.Models.Controls.IFile>();
                List<ForRobot.Models.Controls.File> fileDatas = new List<ForRobot.Models.Controls.File>();
                try
                {
            ...
            else
            {
                ...
                {
                    ForRobot.Models.Controls.IFile newNode = data.Where(x => x.Name == group.Key).ToList().First();
                    if (node == null)
                    {
        ...
        private string SearchPath(string sNameForSearch)
        {
            ...
            {
                string path = ForRobot.Libr.Collections.FileCollection.Search(file, sNameForSearch)?.Path;
