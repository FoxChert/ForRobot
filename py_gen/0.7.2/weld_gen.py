...
# 0.7.2 - реструктуризация наименований параметров детали с сохранением обратной совместимости
...
class DetailParams:
    """
    Класс для хранения, обновления и обеспечения обратной совместимости параметров детали.
    С версии 0.7.2 наименования параметров реструктурированы и используются псевдонимы
    """
    _ALIASES = {
        # --- псевдонимы для основных параметров ---
        'robots_list':'robots',         # задействованные роботы
        'detail_type':'d_type',         # тип детали [d_rect, d_slope_left, d_slope_right, d_trapezoid_top, d_trapezoid_bottom]
        'detail_reverse_deflection':'reverse_deflection', # обратный прогиб детали
        'base_width': 'w',              # ширина настила (горизонтального основания)
        'base_length': 'l',             # длина настила 
        'base_thickness': 't_p',        # толщина настила
        'base_bevel_left': 'd_b1',      # скос настила слева (поведение зависит от типа детали)
        'base_bevel_right': 'd_b2',     # скос настила справа (поведение зависит от типа детали)
        'base_displace':'base_displace',# смещение настила от нулевой точки стола
        'wall_height': 'h',             # высота ребра (вертикальной стенки)
        'wall_thickness': 't_r',        # толщина ребра
        'wall_count': 'edge_count',     # количество рёбер (вертикальных стенок) [устаревший, нужно считать по количеству в списке]
        'walls_list':'d_w',             # список рёбер с параметрами
        'search_offset_start':'d_s1',   # отступ поиска в начале шва
        'search_offset_end':'d_s2',     # отступ поиска в конце шва
        'weld_tech_offset_start':'d_t1',# технологический отступ начала шва
        'weld_tech_offset_end':'d_t2',  # технологический отступ конца шва
        'weld_overlap': 'l_overlap',    # перекрытие швов в месте соединения
        'weld_job':'job',               # номер используемого джоба (ячейки) на источнике
        'weld_velocity':'velocity',     # скорость сварки (обратно пропорциональна получаемому катету)
        'welding_sequence':'welding_sequence', # порядок сварки (в какой очерёдности будут накладываться сварные швы)
        'gantry_radius_weld':'weld_gantry_radius', # расстояние между фланцем робота и позиционера на сварке для расчёта положения позиционера
        'gantry_radius_search':'search_gantry_radius', # расстояние между фланцем робота и позиционера на поисках
        # --- псевдонимы для параметров рёбер ---
        # поперечное расстояние до первого ребра считается от края детали, до следующих - от предыдущего ребра
        'wall_cross_dist_left':'d_dis1',    # поперечное расстояние до ребра по левому краю
        'wall_cross_dist_right':'d_dis2',   # поперечное расстояние до ребра по правому краю
        'wall_long_dist_left':'d_l1',       # продольное расстояние до ребра по левому краю
        'wall_long_dist_right':'d_l2',      # продольное расстояние до ребра по правому краю
        'weld_offset_left':'l_r1',          # отступ шва от левого края ребра (роспуск, выкружка)
        'weld_offset_right':'l_r2'          # отступ шва от правого края ребра (роспуск, выкружка)
    }
    
    # Обратное отображение для repr
    _REVERSE_ALIASES = {v: k for k, v in _ALIASES.items()}

    def __init__(self, params):
        self.__dict__['_ALIASES'] = self._ALIASES
        self.__dict__['_REVERSE_ALIASES'] = self._REVERSE_ALIASES
        self.update(params)

    def _process_value(self, value):
        """Обрабатывает значение (рекурсивно для словарей и списков)"""
        if isinstance(value, dict) and not isinstance(value, DetailParams):
    ...
    def __setattr__(self, name, value):
        if name in ('_ALIASES', '_REVERSE_ALIASES'):
    ...
    def __repr__(self):
        ...
        for key, value in self.__dict__.items():
            if key.startswith('_'):
                continue
            display_key = self._REVERSE_ALIASES.get(key, key)
            display_attrs[display_key] = value
        
        return f"DetailParams({display_attrs})"
    
    def to_dict(self, use_aliases=False, skip_none=True):
        """
        Возвращает данные в виде словаря.
        
        Args:
            use_aliases (bool): Если True, использует псевдонимы для ключей.
            skip_none (bool): Если True, пропускает атрибуты со значением None.
        
        Returns:
            dict: Словарь с данными
        """
        def process_value(v):
            if isinstance(v, DetailParams):
                return v.to_dict(use_aliases, skip_none)
        ...
        for key, value in self.__dict__.items():
            ...
            # Пропускаем None значения если нужно
            if skip_none and value is None:
                continue
                
            # Выбираем ключ для отображения
            if use_aliases:
                display_key = self._REVERSE_ALIASES.get(key, key)
            else:
                display_key = key
            ...
            result[display_key] = process_value(value)
...
def apply_deflection_to_point(point_params):
    ...
    if width == 0 or detail_params.d_type == 'd_rect':
         width_left = sum([d.wall_cross_dist_left for d in detail_params.d_w]) + detail_params.d_w[0].wall_cross_dist_left
         width_right = sum([d.wall_cross_dist_right for d in detail_params.d_w]) + detail_params.d_w[0].wall_cross_dist_right
...
def gen_search_coord(start_point, displace, template, points=3):
    ...
    search_gantry_radius = gantry_param['rob_wrk_radius_search']
    res = [
...
def gen_weld_coord(start_point, end_point, template, departure_displace=ZERO_3D):
    ...
    weld_gantry_radius = gantry_param['rob_wrk_radius_weld']
...
def gen_seq_one_seam(edge_num, side, seam_type):
    ...
    d_l1 = d_w[edge_num].wall_long_dist_left # расстояние до торца ребра слева 
    d_l2 = d_w[edge_num].wall_long_dist_right # расстояние до торца ребра справа
    l_r1 = d_w[edge_num].weld_offset_left # роспуск/отступ слева
    l_r2 = d_w[edge_num].weld_offset_right # роспуск/отступ справа
    ...
    r_y_pos_left = sum([d.wall_cross_dist_left for d in d_w[:edge_num + 1]]) # слева
    r_y_pos_right = sum([d.wall_cross_dist_right for d in d_w[:edge_num + 1]]) # справа
...
def generate(out_dir, main_name, robot_num=1, seam_type='start-to-end'):
    ...
    edge_count = len(detail_params.walls_list)
...
def main():
    ...
    try:
        ...
        logging.info('detail_params:\n' + yaml.dump(detail_params.to_dict(use_aliases=True), default_flow_style=False))
...
detail_params = DetailParams(config_data['default_detail_params'])
c_tmpl = CoordTemplates(detail_params.to_dict())
# временно берём параметры инструмента и позиционера от R1
# TODO использовать индивидуальные параметры для каждого робота
tool = config_data['robots']['R1']['tool']
gantry_param = config_data['robots']['R1']['gantry_param']
base0 = config_data['base']
base = base0.copy()
